...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_load_images_bonus.c                             :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: risattou <risattou@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/02/06 12:07:32 by risattou          #+#    #+#             */
/*   Updated: 2025/02/27 15:28:34 by risattou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	load_sequence1(t_map *all, char *base_path)
{
	mlx_texture_t	*img;
	char			*path;
	int				i;
	...
	i = 0;
	all->enemy = malloc(8 * sizeof(mlx_image_t *));
	if (all->enemy == NULL)
		exit(1);
	while (i <= 7)
	{
		...
		if (img == NULL)
			exit(1);
		all->enemy[i] = mlx_texture_to_image(all->mlx, img);
		mlx_delete_texture(img);
		mlx_resize_image(all->enemy[i], 64, 64);
		i++;
	}
}
...
static void	load_img(t_map *all, t_data *source)
{
	...
	all->text_img = NULL;
	backgrnd = mlx_load_png("Bonus/packages/image.png");
	if (backgrnd == NULL)
		exit(1);
	...
	wall = mlx_load_png("Bonus/packages/bg.png");
	if (wall == NULL)
		exit(1);
...
void	load_main(t_map *all, t_data *source)
{
	...
	all->nbr = 0;
	all->time = 0;
	coins = mlx_load_png("Bonus/packages/12.png");
	if (coins == NULL)
		exit(1);
	all->coins = mlx_texture_to_image(all->mlx, coins);
	mlx_delete_texture(coins);
	mlx_resize_image(all->coins, 48, 48);
	purple = mlx_load_png("Bonus/players/PNG/char/Spells-Effect.png");
	if (purple == NULL)
		exit(1);
	...
	load_img(all, source);
	load_sequence(all, "Bonus/players/PNG/char/0");
	load_sequence1(all, "Bonus/players/PNG/enemy/0");
}
