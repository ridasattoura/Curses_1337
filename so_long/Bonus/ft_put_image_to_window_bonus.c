...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_put_image_to_window_bonus.c                     :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: risattou <risattou@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/02/06 14:57:59 by risattou          #+#    #+#             */
/*   Updated: 2025/02/27 15:53:10 by risattou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	image_put_window1(t_map *all, int x, int y)
{
	int	i;
	...
	while (i < 8)
	{
		mlx_image_to_window(all->mlx, all->enemy[i], x * 64, y * 64);
		i++;
	}
	i = 1;
	while (i < 8)
	{
		all->enemy[i]->enabled = false;
		i++;
	}
}
...
void	ft_movement1(t_map *all, t_data *source)
{
	...
	while (y < source->height)
	{
		...
		while (x < source->width)
		{
			...
			else if (source->map[y][x] == 'F')
				image_put_window1(all, x, y);
