...
/*   Updated: 2025/07/09 03:05:03 by mohchaib         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <minishell.h>

bool	syntax_error(t_token *list, t_shell *shell)
{
	if (!check_balanced_parentheses(list))
	{
	...
	if (!consecutive_operators(list))
		return (false);
	if (!check_parenthesis_logic(list))
		return (false);
	return (true);
}
